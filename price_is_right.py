...
def html_for(log_data):
    """Convert log messages to scrollable HTML."""
...
def setup_logging(log_queue):
    """Set up logging to capture messages in a queue."""
...
class App:
    ...
    def run(self):
        with gr.Blocks(title="DealHunter AI", fill_width=True) as ui:
            log_data = gr.State([])
            ...
            def get_plot():
                try:
                    ...
                    fig = go.Figure()
                    fig.update_layout(title=f"Loading vector DB... ({e})", height=400)
                    return fig
            ...
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:24px"><strong>DealHunter AI</strong>'
                    ' — Autonomous Deal Discovery</div>'
                )
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:14px">'
                    'A proprietary fine-tuned LLM deployed on Modal and a RAG pipeline with a '
                    'frontier model collaborate to send push notifications with great online deals.</div>'
                )
            with gr.Row():
            ...
            # Start the agent loop when the UI loads
            ui.load(
                run_with_logging,
                inputs=[log_data],
            ...
            # Click a deal row to get an alert
